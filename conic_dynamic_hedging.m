...
% final state (hedged)
old_ask = [bid_u,bid_m,bid_d];
[ask,~,delta,~] = bid_tri_tree(S_0,s,r,dt,K,option,'dist',dist,'lambda',lambda,'old_bid',old_ask);
disp(ask)
disp(delta)
...
old_ask = [bid_u,bid_m,bid_d];
[ask,~,~,~] = bid_tri_tree(S_0,s,r,dt,K,option,'dist',dist,'lambda',lambda,'old_bid',old_ask,'hedged',false);
disp(ask)
