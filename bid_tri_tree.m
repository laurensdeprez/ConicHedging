...
%% bid price calculation in trinomial tree
function [bid,bids,delta,deltas,gamma,gammas] = bid_tri_tree(S_0,s,r,T,K,option,varargin)
p = inputParser;
addRequired(p,'S_0');
...
if hedged
    n = delta_precision;
    deltas = linspace(delta_range(1),delta_range(2),delta_precision);
    n_g = gamma_precision;
    gammas = linspace(gamma_range(1),gamma_range(2),gamma_precision);
else
    n = 1;
    deltas = 0;
    n_g = 1;
    ...
    case 'Delta-Gamma'
        ...
        for i=1:n
            for j=1:n_g
                ...
                dist_cdf = distortion(cumsum(ps(I)),dist,lambda);
                dist_sorted_ps = [dist_cdf(1),diff(dist_cdf)];
                bids(j,i) = exp(-r*T)*sum((sorted_pi).*(dist_sorted_ps));
            end
        end
        bid = max(max(bids));
        [j,i] = find(bids==max(max(bids)));
        delta = deltas(i);
        gamma = gammas(j);
    otherwise
